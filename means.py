...
def load_train_data():
    ...
    df_train = pd.read_csv(dir_train, usecols=[ 'Semana', 'Cliente_ID', 'Producto_ID', 'Demanda_uni_equil'], 
                           dtype  = {'Semana': 'uint8',
                                     'Producto_ID':'uint32',
                                     'Cliente_ID' : 'uint32',
...
def load_test_data():
    ...
    df_test = pd.read_csv(dir_test, usecols=[ 'id', 'Cliente_ID', 'Producto_ID' ],
                          dtype  = { 'Semana': 'uint8',
                                     'Producto_ID':'uint32',
                                     'Cliente_ID' : 'uint32',
                                     'id':'uint32'})
...
n_semanas = 1 # To choose a subset of the training dataset
...
meanPC = df.groupby(['Cliente_ID', 'Producto_ID'], as_index=False).mean()
...
test['Demanda_uni_equil'] = test.Demanda_uni_equil_x
